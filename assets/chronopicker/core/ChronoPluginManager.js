export class ChronoPluginManager {
	constructor(context) {
	...
	install(plugins = []) {
		for (const pluginDefinition of plugins) {
			...
			if (!plugin || !plugin.name) {
				throw new Error('ChronoPicker plugins require a unique name.');
			}

			if (this.plugins.some((installedPlugin) => installedPlugin.name === plugin.name)) {
				throw new Error(`ChronoPicker plugin already installed: ${plugin.name}`);
			}
